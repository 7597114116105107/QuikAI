...
const WriteArtical = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      ...
      const {data} = await axios.post('/api/ai/dev/generate-article', {prompt, length: selectedLength.length}, {
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <br />
        <button disabled={loading} className ='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 mt-6 text rounded-lg cursor-pointer'>
          {
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
          ...
          {!content ? (
            ...
            <div className='mt-3 h-full overflow-y-scroll text-sm text-slate-600'>
              <div className='reset-tw whitespace-pre-line'>
